...
#### 案例：同一个多链钱包内多种加密货币的处理

在Rust中，我们可以使用泛型来表示同一个钱包内多种加密货币的处理。下面是一个简单的示例，展示了如何使用泛型来创建一个可以处理多种加密货币的钱包。
...
trait Cryptocurrency {
    ...
    fn send(&mut self, amount: u64, to: &str);
    // 可以添加其他方法，如接收、交易等
}

// 创建一个比特币结构体，实现 Cryptocurrency trait
struct Bitcoin {
    balance: u64,
}

impl Cryptocurrency for Bitcoin {
    fn get_balance(&self) -> u64 {
        self.balance
    }
    ...
    fn send(&mut self, amount: u64, to: &str) {
        self.balance -= amount;
        println!("Sent {} Bitcoin to {}", amount, to);
    }
}

// 创建一个以太坊结构体，实现 Cryptocurrency trait
struct Ethereum {
    balance: u64,
}

impl Cryptocurrency for Ethereum {
    fn get_balance(&self) -> u64 {
        self.balance
    }
    ...
    fn send(&mut self, amount: u64, to: &str) {
        self.balance -= amount;
        println!("Sent {} Ethereum to {}", amount, to);
    }
}

// 创建一个泛型钱包结构体，可以容纳多种加密货币
struct Wallet {
    cryptocurrencies: HashMap<String, Box<dyn Cryptocurrency>>,
}
...
impl Wallet {
    ...
    fn add_crypto(&mut self, name: &str, cryptocurrency: Box<dyn Cryptocurrency>) {
        self.cryptocurrencies.insert(name.to_string(), cryptocurrency);
    }

    fn get_balance(&self, name: &str) -> Option<u64> {
        if let Some(crypto) = self.cryptocurrencies.get(name) {
            Some(crypto.get_balance())
        } else {
            None
        }
    }

    fn send(&mut self, name: &str, amount: u64, to: &str) {
        if let Some(crypto) = self.cryptocurrencies.get_mut(name) {
            crypto.send(amount, to);
        } else {
            println!("Crypto not found: {}", name);
        }
    }
}
...
fn main() {
    ...
    wallet.add_crypto("Bitcoin", Box::new(Bitcoin { balance: 100 }));
    wallet.add_crypto("Ethereum", Box::new(Ethereum { balance: 50 }));

    wallet.send("Bitcoin", 20, "Alice");
    println!("Bitcoin balance: {}", wallet.get_balance("Bitcoin").unwrap_or(0));

    wallet.send("Ethereum", 10, "Bob");
    println!("Ethereum balance: {}", wallet.get_balance("Ethereum").unwrap_or(0));
}
...
```text
Sent 20 Bitcoin to Alice
Bitcoin balance: 80
Sent 10 Ethereum to Bob
Ethereum balance: 40
```

在此示例中，我们使用了一个 `HashMap` 来容纳多种加密货币，每种加密货币都由实现了 `Cryptocurrency` trait 的具体结构体表示。通过 `add_crypto` 方法，我们可以向钱包添加不同类型的加密货币，并通过名称来区分它们。`get_balance` 方法允许查询特定加密货币的余额，而 `send` 方法允许发送特定加密货币。这种方式使得钱包能够同时容纳多种加密货币。
...
这个示例演示了如何在处理金融数据时使用终身泛型来管理文件资源的生命周期，以避免资源泄漏。在实际应用中，我们可能需要更复杂的金融数据结构和处理逻辑，但终身泛型的概念仍然是相同的，用于确保资源的正确管理。
...
# Chapter 26 - 智能指针与内存结构

#  Chapter 27 - Unsafe 

# Chapter 28 - Cerebro 引擎系统

# Chapter 29 - Logger日志系统
