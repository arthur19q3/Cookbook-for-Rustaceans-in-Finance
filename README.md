...
Polars 是一个高性能的数据处理库，具备以下几个显著的优势：

#### 1. **高性能**
Polars 使用 Rust 编写，紧贴机器设计，没有外部依赖，性能卓越。

#### 2. **I/O 支持**
Polars 对本地、云存储和数据库的所有常见数据存储层提供一流支持。

#### 3. **直观的 API**
Polars 提供直观的 API，允许开发者以自然的方式编写查询，内部查询优化器会确定最有效的执行方式。

#### 4. **Out of Core**
流式 API 允许在不需要将所有数据同时加载到内存中的情况下处理结果。

#### 5. **并行处理**
Polars 利用机器的多核 CPU，无需额外配置即可分配工作负载。

#### 6. **向量化查询引擎**
使用 Apache Arrow 列式数据格式，以向量化方式处理查询，优化 CPU 使用。

#### 7. **LazyMode**
支持延迟计算模式（LazyMode），可以通过链式调用优化性能和资源使用。

#### 8. **PyO3 支持**
通过 PyO3，Polars 提供了对 Python 的强大支持，使得研究人员可以方便地使用 Python 进行数据分析。
