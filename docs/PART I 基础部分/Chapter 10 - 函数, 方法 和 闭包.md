...
2. **返回函数：** 类似地，你可以编写函数，以函数作为它们的返回值。这种函数通常被称为工厂函数，因为它们返回其他函数的实例。
   ...
   还有一种写法, 可以省去对返回值签名 impl 的记忆:
   ```rust
   type FnPtr = Box<dyn Fn(i32) -> i32>;

   fn create_multiplier(factor: i32) -> FnPtr {
       //"impl Fn(i32) -> i32 " 是返回类型的标记，它用于指定闭包的类型签名。
       Box::new(move |x| x * factor)
   }
   
   fn main() {
       let double: FnPtr = create_multiplier(2);
       let triple: FnPtr = create_multiplier(3);
       println!("double 5 = {}", double(5));
       println!("triple 5 = {}", triple(5));
   }
   ```
   之前 create_multiplier 函数返回的是一个闭包，而不是一个函数指针。在 Rust 中，闭包和函数指针是不同的类型，尽管它们在某些情况下可以互换使用。函数指针是一个指向函数的指针，它可以被调用，就像普通函数一样。闭包是一个匿名函数，它可以捕获其环境中的变量。闭包也可以被调用，但它们不能被转换为函数指针，除非它们不捕获任何变量。create_multiplier 函数返回的闭包捕获了 factor 变量，因此它不能被转换为函数指针。这就是为什么编译器会报错，提示期望的是一个函数指针，但找到了一个闭包。

   为了解决这个问题，可以将 create_multiplier 函数的返回类型改为 Box<dyn Fn(i32) -> i32>，这是一个 trait 对象，它可以存储任何实现了 Fn(i32) -> i32 trait 的类型，包括闭包。这样，create_multiplier 函数就可以返回一个闭包，而不会导致类型不匹配的错误。

4. **迭代器和高阶函数：** Rust的标准库提供了丰富的迭代器方法，这些方法允许你对集合(如数组、向量、迭代器等)进行高级操作，例如`map`、`filter`、`fold`等。这些方法都可以接受函数闭包作为参数，使你能够非常灵活地处理数据。
